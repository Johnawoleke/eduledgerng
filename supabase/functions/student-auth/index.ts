...
//   3. Only PUBLISHED fees are ever returned.
...
serve(async (req) => {
  ...
  try {
    ...
    // --- Fees (PUBLISHED only) + settled payments ----------------------------
    ...
    if (feesError) {
      console.error("student-auth fees:", feesError.message);
      return json({ error: "Something went wrong. Please try again." }, 500);
    }
    ...
    const feeItems = (classFees || []).map((cf: { id: string; name: string; amount: number; session_id?: string; term_id?: string }) => {
      const paid = Math.min(
        sumPaidForFee(settledPayments, { id: cf.id, name: cf.name }),
        Number(cf.amount)
      );
      return {
        id: cf.id,
