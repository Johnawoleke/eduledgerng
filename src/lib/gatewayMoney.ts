// PRODUCTION gateway money math. This is the live checkout maths — not the
// modelling library at src/lib/gatewayFees.ts, which exists to compare providers
// we might use. Only what we actually charge belongs here.
//
// Mirrored for Deno at supabase/functions/_shared/gatewayMoney.ts; the two must
// stay identical below their headers (asserted by src/lib/gatewayMoney.test.ts),
// because if they drift the amount a parent is charged and the amount they are
// credited stop agreeing.
//
// Paystack is the only gateway. Squad was routed here from 2026-08-06 and
// removed on 2026-08-17 without ever having settled a payment — no school
// completed sub-merchant provisioning, so nothing historical depends on it.
//
// The per-gateway shape below is kept even though there is one entry, because
// it is what makes Paystack for Education (0.7% capped ₦1,500) a rate constant
// plus a selectGateway() threshold rather than a restructure. payments.gateway
// records the provider per row for the same reason.
//
// The invariant: the school receives the EXACT fee it set. The platform fee and
// the gateway's charge are both added on top and borne by the parent.
//
